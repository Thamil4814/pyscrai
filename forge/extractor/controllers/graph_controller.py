...
from forge.extractor.ui.theme import BG_CARD, BORDER_MEDIUM
...
class GraphController:
    ...
    def build_view(self) -> ft.Control:
        ...
        async def on_view_graph(e):
            ...
            if not graph_available:
                await self.app_controller.push_log("No graph data available to visualize", "warning")
                return
            ...
            try:
                ...
                if html_path and html_path.exists():
                    ...
                    if url:
                        if self._launch_browser(url):
                            await self.app_controller.push_log(f"Graph visualization opened in browser", "success")
                        else:
                            await self.app_controller.push_log(f"Could not open browser. URL: {url}", "warning")
                    else:
                        # Fallback to file:// URL
                        file_url = f"file://{html_path.absolute()}"
                        if self._launch_browser(file_url):
                            await self.app_controller.push_log(f"Graph visualization opened in browser", "success")
                        else:
                            await self.app_controller.push_log(f"Could not open browser. File: {html_path}", "warning")
                else:
                    await self.app_controller.push_log("Failed to generate graph visualization", "error")
        ...
        async def on_export_html(e):
            ...
            if not graph_available:
                await self.app_controller.push_log("No graph data available to export", "warning")
                return
            
            try:
                # For now, just open the graph - in a full implementation, we'd use a file dialog
                await on_view_graph(e)
                await self.app_controller.push_log("Graph exported (opened in browser)", "success")
            except Exception as ex:
                logger.error(f"Error exporting graph: {ex}")
                await self.app_controller.push_log(f"Error exporting graph: {str(ex)}", "error")
        ...
        async def on_layout_change(e):
            ...
            # Only log the change; do not update the whole page unless UI components change
            await self.app_controller.push_log(f"Layout changed to {self._current_layout}", "info")
        ...
        # Stats panel
        stats_panel = ft.Container(  # type: ignore[call-arg]
            bgcolor=BG_CARD,
            padding=16,
            border_radius=8,
        ...
        # Controls
        layout_dropdown = ft.Dropdown(  # type: ignore[call-arg]
            label="Layout Algorithm",  # type: ignore[arg-type]
            ...
            border_color=BORDER_MEDIUM,  # type: ignore[arg-type]
            width=200,
        )
        ...
        # Content
        if not graph_available:
            content = ft.Container(  # type: ignore[call-arg]
                padding=40,
                content=ft.Column(
                    [
        ...
        return ft.Container(  # type: ignore[call-arg]
            padding=20,
            content=ft.Column(  # type: ignore[arg-type]
                [
                    header,
    ...
    def _apply_layout(
        ...
        # Add all entity nodes with their attributes
        for entity in entities:
            entity_id = entity.get("id")
            if entity_id:
                G.add_node(
    ...
    def _launch_browser(self, url: str) -> bool:
        """Open a URL in the default browser, robustly handling WSL2 and fallback scenarios."""
        import shutil
        import subprocess
        import webbrowser
        ...
        for browser_name, use_app_mode in browsers:
            try:
                cmd = [browser_name, f'--app={url}'] if use_app_mode else [browser_name, url]
    ...
    async def _serve_html_file(self, html_path: Path) -> Optional[str]:
        """Start a simple HTTP server to serve the HTML file, using a context manager for safety."""
        try:
            ...
            # Use context manager to ensure immediate cleanup on failure
            try:
                http_server = HTTPServer(('127.0.0.1', port), Handler)
            except Exception as e:
    ...
    def _generate_and_save_graph(
        ...
        # Save to file using project path from app controller for consistency
        try:
            # Determine project root - use app_controller if it has project context
            # Otherwise fallback to calculating from current file location
