...
class AgentForgePage:
    ...
    def render(self):
        ...
        try:
            ...
            entities_df = self.session.db.get_promotable_entities()
            active_agents_df = self.session.db.get_active_agents()
    ...
    def _render_agent_editor(self):
        ...
        with st.expander("📄 Entity Context", expanded=False):
            if entity_data:
                ...
                try:
                    attrs = json.loads(entity_data[3]) # attributes_json
                    for key, value in attrs.items():
                        st.markdown(f"• **{key}:** {value}")
                except:
                    st.markdown(entity_data[3])
            else:
...
- Develop and evolve based on experiences"""
    ...
    def _deactivate_agent(self, agent_id: str):
        """Deactivate an agent using repository methods."""
        if not self.session.db:
            st.error("No database connection.")
            return

        try:
            self.session.db.delete_agent(agent_id)
