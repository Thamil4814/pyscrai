...
class ProjectController:
    ...
    def __init__(self, app_state: AppState, session_manager: SessionManager, page: ft.Page):
    ...
    def build_view(self) -> ft.Control:
        ...
        async def on_open_project(e):
            ...
            # Open file dialog
            # Use project root relative path for projects directory
            ...
            if file_path:
                await self.app_controller.push_log(f"Opening project: {file_path}", "info")
                await self.session_manager.open_project(file_path)
            else:
                await self.app_controller.push_log("Project open cancelled", "info")
        ...
        async def on_save_project(e):
            ...
            if file_path:
                await self.app_controller.push_log(f"Saving project to: {file_path}", "info")
                await self.session_manager.save_project(file_path)
            else:
                await self.app_controller.push_log("Project save cancelled", "info")
            
        async def on_reset_click(e):
            """Reset the current project (clear all data)."""
            await self.app_controller.push_log("Resetting project...", "warning")
            await self.session_manager.clear_session()

        # --- Configuration Inputs ---
        
        api_key_input = ft.TextField(  # type: ignore[call-arg]
            label="OpenRouter API Key",
            password=True,  # type: ignore[arg-type]
            can_reveal_password=True,  # type: ignore[arg-type]
            value=os.getenv("OPENROUTER_API_KEY", ""),  # type: ignore[arg-type]
            border_color="rgba(255,255,255,0.2)",  # type: ignore[arg-type]
        )
        
        model_dropdown = ft.Dropdown(  # type: ignore[call-arg]
            label="Default Model",  # type: ignore[arg-type]
            ...
            value=os.getenv("OPENROUTER_MODEL", "anthropic/claude-3-sonnet"),  # type: ignore[arg-type]
            border_color="rgba(255,255,255,0.2)",  # type: ignore[arg-type]
        )
        ...
        def on_save_config(e):
            ...
            asyncio.create_task(self.app_controller.push_log("Configuration updated (Runtime only)", "success"))
    ...
    def new_project(self, e):
        ...
        asyncio.create_task(self.app_controller.push_log("Starting new project...", "info"))
