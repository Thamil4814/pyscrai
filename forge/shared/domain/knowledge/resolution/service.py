...
class EntityResolutionService(BaseLLMService):
    ...
    async def handle_entity_extracted(self, payload: EventPayload):
        ...
        # Filter entities to those present in the original document content to avoid template noise
        if document_content:
            filtered_entities = [
                e for e in entities
                ...
                and e["text"].strip()
                and e["text"] in document_content
            ]
            # Fallback to original entities if filtering removed everything
            entities = filtered_entities or entities
        
        # Cache entities by document (after filtering)
